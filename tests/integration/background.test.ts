...
describe("background: 放置されたnotifiedレコードの掃除（実Chromeスモークテスト監査で発見）", () => {
  it("通知後24時間以上ユーザーが何も操作しなかったnotifiedレコードは、整合処理で削除される", async () => {
    await seedState({
      schemaVersion: 1,
      timersById: {
        r1: fakeRecord({
  ...
  it("通知後24時間未満のnotifiedレコードは削除されない", async () => {
    await seedState({
      schemaVersion: 1,
      timersById: {
        r1: fakeRecord({
...
describe("background: ensureReconciled失敗時の各リスナーの耐性（実Chromeスモークテスト監査で発見）", () => {
  it("tabs.onRemoved経由でensureReconciled()が失敗しても、未処理rejectionにならない", async () => {
    chromeExtra.storage.session.get.callsFake(() => Promise.reject(new Error("storage unavailable")));
    await loadBackgroundFresh();
    ...
    // ここまで到達すれば（テストランナーが未処理rejectionでプロセスごと
    // 失敗しなければ）意図どおり。
